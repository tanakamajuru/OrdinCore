...
| **OpenAI / Groq** (`OPENAI_API_KEY`, `NARRATIVE_API_KEY`) | Narrative/text generation — **NOT ACTIVE in production**: no API keys configured, feature disabled per DPIA R3 (option c). No data is sent. | n/a while disabled (would be **potentially special‑category** if enabled) | **USA** (if enabled) | **Not a live sub‑processor.** Before enabling: DPA (zero‑retention + no‑training) + UK IDTA/Addendum + TRA + input de‑identification + DPIA update. |
