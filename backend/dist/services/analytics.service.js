...
class AnalyticsService {
    ...
    async getMultiHouseRiskTrends(company_id, days = 42) {
        const result = await (0, database_1.query)(`SELECT 
       ...
       ORDER BY date, house_name`, [company_id]);
        ...
        const houseNames = Array.from(new Set(result.rows.map((row) => row.house_name)));
    ...
    async getTrends(company_id) {
        const multiHouseTrends = await this.getMultiHouseRiskTrends(company_id, 42);
        const incidentsResult = await (0, database_1.query)(`SELECT created_at FROM incidents WHERE company_id = $1 AND created_at >= NOW() - INTERVAL '42 days'`, [company_id]);
        const escalationsResult = await (0, database_1.query)(`SELECT created_at FROM escalations WHERE company_id = $1 AND created_at >= NOW() - INTERVAL '42 days'`, [company_id]);
        const now = new Date();
        ...
        escalationsResult.rows.forEach((row) => {
            const d = new Date(row.created_at);
            const week = weeks.find(w => d >= w.start && d <= w.end);
            if (week)
                week.escalations++;
