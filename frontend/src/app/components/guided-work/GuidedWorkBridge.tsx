...
export function GuidedWorkBridge() {
  ...
  const next = async () => {
    ...
    try {
      ...
      if (err?.response?.status === 409) setMessage('This task is still active. Complete the canonical review before continuing.');
  ...
  return <div className="fixed bottom-4 left-4 right-4 z-[80] mx-auto max-w-3xl rounded-xl border border-border bg-card shadow-lg p-3 flex flex-wrap items-center gap-3">
    ...
    <div className="flex-1 min-w-[180px]"><div className="text-sm font-semibold text-foreground">Guided Work</div><div className="text-xs text-muted-foreground">Complete the work on this existing screen, then continue. This bar does not change or close governance records.</div>{message&&<div className="text-xs text-amber-700 font-medium mt-1">{message}</div>}</div>
    ...
    <button type="button" onClick={next} disabled={loading} className="min-h-10 px-3 rounded-lg bg-primary text-primary-foreground text-sm font-semibold inline-flex items-center gap-1.5 disabled:opacity-60">{loading?'Loading…':'Continue to next'} <ArrowRight size={15}/></button>
