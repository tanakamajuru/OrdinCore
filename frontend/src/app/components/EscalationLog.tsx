...
export function EscalationLog() {
  ...
  const loadEscalations = async () => {
    try {
      ...
      // Convert dashboard activities to escalation format
      const escalationData = dashboardData.recentActivities
        ...
        .map((activity: any, index: number) => ({
          ...
          riskId: activity.id || `R-2024-${index + 89}`,
          risk: activity.description,
          escalatedBy: activity.createdBy || user?.name || "Unknown",
          escalatedTo: activity.escalatedTo || "Director",
          responseTime: activity.responseTime || "1 hour",
          action: activity.action || "Under review"
        }));
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">Loading escalation log...</p>
        </div>
      </div>
    );
  }

  // Mock data for fallback if no escalations found
