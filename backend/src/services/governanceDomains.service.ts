...
export const governanceDomainsService = {
  ...
  /**
   * Returns the active domains for the company's sector, each with its selectable
   * signals (the per-domain dropdown) and threshold rule.
   */
  async getDomainsForCompany(company_id: string | null) {
    const sector = await this.getSector(company_id);
