...
export function RegisteredManagerDashboard() {
  ...
  const loadOversightData = async (selectedHouseId?: string) => {
    try {
      ...
      if (myHouse) {
        ...
        setRiskCandidates(Array.isArray(candidatesRes.data) ? candidatesRes.data : candidatesRes.data?.data || []);
  ...
  const sectionClass = "bg-card border-4 border-border p-8 shadow-[12px_12px_0px_rgba(0,0,0,1)]";
  const titleClass = "text-xl font-black uppercase italic text-primary mb-6 tracking-tighter flex items-center gap-3";
  const badgeClass = "bg-primary text-primary-foreground px-3 py-1 text-xs font-black uppercase italic tracking-widest";
    ...
    <div className="min-h-screen bg-[#FDFCFB]">
      ...
      {riQueries.length > 0 && (
        <div className="p-6 w-full max-w-6xl mx-auto mt-8">
            <div className="border-4 border-border bg-card shadow-[8px_8px_0px_rgba(255,0,0,1)] p-6">
                <div className="flex items-center gap-3 mb-6 text-destructive">
                    <Shield className="w-8 h-8" />
                    <h2 className="text-lg font-black uppercase italic tracking-tighter">Outstanding RI Governance Queries</h2>
                </div>
                        ...
                        <div key={q.id} className="p-6 border-4 border-border bg-muted/10">
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <button 
                                    ...
                                    className="bg-primary text-primary-foreground px-6 py-3 font-black uppercase italic tracking-widest hover:bg-primary transition-all"
                                >
                                    Justify
                                </button>
                            </div>
                            <div className="bg-card border-2 border-border p-4 italic text-xs text-muted-foreground">
      ...
      <div className={`p-6 w-full ${isDeputyCover || riQueries.length > 0 ? 'pt-8' : 'pt-24'} max-w-6xl mx-auto`}>
          ...
          {[
            ...
            <div key={s.id} className="flex flex-col items-center gap-2">
              <button 
                ...
                className={`w-12 h-12 flex items-center justify-center border-4 border-border font-black transition-all ${
                  activeSection === s.id ? 'bg-primary text-primary-foreground shadow-[4px_4px_0px_rgba(0,0,0,1)]' : 
        ...
        <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4 border-b-4 border-primary pb-6">
          <div>
            <h1 className="text-2xl font-black text-primary tracking-tighter uppercase italic leading-none">Oversight Board</h1>
            <p className="text-muted-foreground font-bold mt-2 tracking-wide uppercase text-sm">
          ...
          <div className="flex flex-col md:flex-row gap-4 items-end">
            {availableHouses.length > 1 && (
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Select Service</label>
                <select 
                  value={house?.id} 
                  onChange={(e) => loadOversightData(e.target.value)}
                  className="bg-card border-2 border-primary px-4 py-2 font-black uppercase italic text-sm outline-none cursor-pointer"
                >
              ...
              <div className="space-y-4">
                {highPrioritySignals.length > 0 ? highPrioritySignals.map(s => (
                  <div key={s.id} className="p-6 border-4 border-border bg-muted/20 flex justify-between items-center">
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        <span className="bg-destructive text-primary-foreground px-3 py-1 text-[10px] font-black uppercase italic tracking-widest">{s.severity}</span>
                    ...
                    <button 
                      ...
                      className="bg-primary text-primary-foreground px-8 py-3 font-black uppercase italic tracking-widest hover:bg-primary transition-all shadow-[4px_4px_0px_rgba(255,0,0,1)]"
                    >
                      Review
                    </button>
                  </div>
                  ...
                  <div className="py-20 text-center border-4 border-dashed border-border">
                     <p className="text-lg font-black uppercase italic text-muted-foreground">All Priority Signals Triaged</p>
                     <button 
                      onClick={() => { setCompletedSection(Math.max(completedSection, 1)); setActiveSection(2); }}
                      className="mt-6 px-10 py-4 bg-primary text-primary-foreground font-black uppercase italic tracking-widest shadow-[8px_8px_0px_rgba(0,0,0,1)]"
                    >
                      Next: Patterns
                    </button>
                  </div>
                   ...
                   <div key={c.id} className="p-6 border-4 border-border">
                      ...
                      <div className="bg-primary h-2 w-full mb-4">
                       ...
                       <button 
                        ...
                        className="w-full py-3 border-2 border-border font-black uppercase tracking-widest hover:bg-primary hover:text-primary-foreground transition-all"
                      >
                        Analyse Cluster
                      </button>
                   </div>
                 )) : (
                    <div className="col-span-2 py-20 text-center border-4 border-dashed border-border">
                      <p className="text-lg font-black uppercase italic text-muted-foreground">No emerging patterns requiring review</p>
                      <button 
            ...
            <div className={sectionClass}>
              ...
              <div className="space-y-4">
                {riskCandidates.length > 0 ? riskCandidates.map(rc => (
                  <div key={rc.id} className="p-6 border-4 border-border bg-destructive/5">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <span className="bg-destructive text-primary-foreground px-3 py-1 text-[10px] font-black uppercase italic tracking-widest mb-2 inline-block">
                          {rc.candidate_type}
                        </span>
                        <h3 className="text-lg font-black uppercase italic tracking-tighter">{rc.risk_domain} Risk Candidate</h3>
                      </div>
                    ...
                    <div className="flex gap-4">
                       <button 
                        ...
                        className="flex-1 py-4 bg-destructive text-primary-foreground font-black uppercase italic tracking-widest hover:bg-primary transition-all shadow-[4px_4px_0px_rgba(0,0,0,1)]"
                      >
                        Promote to Risk Register
                      </button>
                      <button className="px-8 py-4 border-4 border-border font-black uppercase italic tracking-widest hover:bg-muted transition-all">
                        Dismiss
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="py-20 text-center border-4 border-dashed border-border">
                    <p className="text-lg font-black uppercase italic text-muted-foreground">No pending risk candidates</p>
                    <button 
            ...
            <div className={sectionClass}>
              ...
              <div className="text-center">
                <button 
                  ...
                  className="px-10 py-4 bg-primary text-primary-foreground font-black uppercase italic tracking-widest shadow-[8px_8px_0px_rgba(0,0,0,1)]"
                >
                  Next: Sign-Off
                </button>
              </div>
            ...
            <div className={sectionClass}>
              ...
              <div className="mb-8">
                ...
                <textarea 
                  ...
                  className="w-full h-64 p-6 border-4 border-border text-xl font-medium focus:outline-none focus:ring-0 shadow-[8px_8px_0px_rgba(0,0,0,1)]"
              ...
              <button 
                ...
                className="w-full py-6 bg-primary text-primary-foreground font-black uppercase italic text-lg tracking-widest shadow-[12px_12px_0px_rgba(0,0,0,1)] hover:shadow-[6px_6px_0px_rgba(0,0,0,1)] hover:translate-x-1 hover:translate-y-1 transition-all disabled:opacity-50"
              >
        ...
        <div className="mt-12 flex justify-between items-center pb-20">
           <button 
            ...
            className="px-10 py-4 border-4 border-border font-black uppercase italic tracking-widest disabled:opacity-20"
           >
             Previous
           </button>
           <button 
            ...
            className="px-10 py-4 bg-primary text-primary-foreground font-black uppercase italic tracking-widest disabled:opacity-20 shadow-[6px_6px_0px_rgba(255,0,0,1)]"
           >
             Next
           </button>
        </div>
