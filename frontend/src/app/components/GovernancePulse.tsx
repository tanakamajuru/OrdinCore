  ...
  if (submitted) return (
    <div className="min-h-screen bg-background flex items-center justify-center pt-20">
      <div className="text-center max-w-md p-8 bg-card border-2 border-primary shadow-2xl">
        ...
        <h2 className="text-3xl  text-foreground mb-2  tracking-tighter uppercase">SIGNAL RECORDED</h2>
        <p className="text-muted-foreground  mb-6">The observation has been logged for management review.</p>
        <button onClick={() => navigate('/dashboard')} className="w-full py-3 bg-primary text-primary-foreground  uppercase  tracking-widest hover:bg-primary/90 transition-all">
          Return to Dashboard
        </button>
      </div>
    </div>
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="p-6 md:px-12 lg:px-24 pt-28 pb-20 max-w-5xl mx-auto">
        <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4 border-b-4 border-primary pb-6">
          <div>
            <h1 className="text-5xl  text-foreground tracking-tighter uppercase  leading-none">Governance Pulse</h1>
            <p className="text-muted-foreground  mt-2 tracking-wide uppercase text-sm">Daily Structured Observation Form</p>
          </div>
          <div className="flex gap-4">
             <div className="bg-card border-2 border-border px-4 py-2 flex items-center gap-3">
               <Calendar className="w-5 h-5 text-primary" />
               <span className=" text-lg">{new Date().toLocaleDateString('en-GB')}</span>
             </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-12">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <input 
                  ...
                  className="bg-card border-2 border-border p-3  focus:border-primary outline-none"
                />
                <input 
                  type="time" 
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {['Incident', 'Concern', 'Observation', 'Safeguarding', 'Medication', 'Staffing', 'Environment', 'Positive signal'].map(type => (
                  <button
                    ...
                    className={`py-2 px-1 text-[10px]  uppercase border-2 transition-all ${formData.signal_type === type ? 'bg-primary text-primary-foreground border-primary' : 'bg-card border-border hover:border-primary/50'}`}
                  >
                    {type}
                  </button>
                ))}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {['Behaviour', 'Medication', 'Staffing', 'Physical health', 'Mental health', 'Safeguarding', 'Environment', 'Governance'].map(domain => (
                  <button
                    ...
                    className={`py-2 px-1 text-[10px]  uppercase border-2 transition-all ${formData.risk_domain.includes(domain) ? 'bg-primary text-primary-foreground border-primary shadow-md scale-[1.02]' : 'bg-card border-border hover:border-primary/50 opacity-60'}`}
                  >
                    {domain}
                  </button>
                ))}
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-2">
              <label className="flex items-center gap-2 text-xs  uppercase tracking-widest text-primary">
              ...
              <textarea 
                ...
                className="w-full bg-card border-2 border-primary/30 p-4  focus:border-primary outline-none resize-none"
              />
            </div>
          </div>
              ...
              <div className="flex flex-col gap-1">
                {['Low', 'Moderate', 'High', 'Critical'].map(level => (
                  <button
                    ...
                    className={`py-2 px-4 text-xs  uppercase border-2 text-left transition-all ${formData.severity === level ? 'bg-primary text-primary-foreground border-primary' : 'bg-card border-border hover:border-primary/50'}`}
                  >
                    {level}
                  </button>
                ))}
              ...
              <div className="flex flex-col gap-1">
                {['Yes', 'No', 'Unsure'].map(val => (
                  <button
                    ...
                    className={`py-2 px-4 text-xs  uppercase border-2 text-left transition-all ${formData.has_happened_before === val ? 'bg-primary text-primary-foreground border-primary' : 'bg-card border-border hover:border-primary/50'}`}
                  >
                    {val}
                  </button>
                ))}
              ...
              <div className="flex flex-col gap-1">
                {['None', 'Possible repeat', 'Clear repeat', 'Escalating'].map(val => (
                  <button
                    ...
                    className={`py-2 px-4 text-xs  uppercase border-2 text-left transition-all ${formData.pattern_concern === val ? 'bg-primary text-primary-foreground border-primary' : 'bg-card border-border hover:border-primary/50'}`}
                  >
                    {val}
                  </button>
                ))}
              ...
              <div className="flex flex-col gap-1">
                {['No escalation', 'Manager review', 'Urgent review', 'Immediate escalation'].map(val => (
                  <button
                    ...
                    className={`py-2 px-4 text-xs  uppercase border-2 text-left transition-all ${formData.escalation_required === val ? 'bg-primary text-primary-foreground border-primary' : 'bg-card border-border hover:border-primary/50'}`}
                  >
                    {val}
                  </button>
                ))}
          ...
          <div className="space-y-2">
            ...
            <div className="bg-muted border-2 border-dashed border-border p-8 text-center hover:border-primary/50 transition-all cursor-pointer group">
              ...
              <p className="text-sm  text-muted-foreground uppercase">Upload observation photos or documents</p>
            </div>
          </div>
          ...
          <div className="pt-10 border-t-4 border-border flex justify-end gap-6">
            <button 
              ...
              className="px-8 py-4 border-2 border-border  uppercase tracking-widest hover:bg-muted transition-all"
            >
              Cancel
            </button>
            <button 
