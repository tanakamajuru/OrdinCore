...
class UsersService {
    async create(company_id, data) {
        ...
        const user = await users_repo_1.usersRepo.create({ company_id, email: data.email, password_hash, first_name: data.first_name, last_name: data.last_name, role: data.role });
    ...
    async findAll(company_id, page = 1, limit = 50) {
        ...
        const [users, total] = await Promise.all([
            users_repo_1.usersRepo.findByCompany(company_id, limit, offset),
    ...
    async update(id, company_id, data) {
        ...
        if (!user || user.company_id !== company_id)
            throw new Error('User not found');
        const updated = await users_repo_1.usersRepo.update(id, data);
