...
import { Settings, Tags, Activity, AlertTriangle, ClipboardList, FileSearch, Plus, Check, X, Lock } from "lucide-react";
      ...
      <div className="p-6 w-full pt-20 max-w-7xl mx-auto">
        ...
        {!isSuper && (
          <div className="flex items-center gap-2 text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mb-4">
            <Lock size={13} /> Read-only. Configuration is shared across all services of a sector — only a Super Admin can change it.
          </div>
        )}
            ...
            <div className="overflow-x-auto">
              {tab === "domains" && (
                <table className="w-full">
                  <thead><tr><th className={th}>Domain</th><th className={th}>Description</th><th className={th}>Order</th><th className={th}>Status</th></tr></thead>
                  <tbody>
                    {adding && (
                      <tr className="bg-primary/5">
                        ...
                        <td className={td} colSpan={2}><input className="w-full px-2 py-1 bg-input-background border border-border rounded text-sm" placeholder="Description" value={adding.description} onChange={(e) => setAdding({ ...adding, description: e.target.value })} /></td>
                        <td className={td}>
