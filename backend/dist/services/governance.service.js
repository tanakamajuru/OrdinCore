...
class GovernanceService {
    async createPulse(company_id, data) {
       ...
       VALUES ($1,$2,$3,$4,'pending',$5) RETURNING *`, [id, company_id, data.house_id, data.template_id, data.due_date]);
    ...
    async submitAnswers(pulse_id, company_id, user_id, answers) {
        ...
        await (0, database_1.query)(`UPDATE governance_pulses SET status = 'completed', completed_at = NOW(), completed_by = $1, compliance_score = $2, updated_at = NOW()
    ...
    async getTemplates(company_id) {
       ...
       GROUP BY gt.id
       ORDER BY gt.created_at DESC`, [company_id]);
        return result.rows;
    }
