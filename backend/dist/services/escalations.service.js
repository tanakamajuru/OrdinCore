...
exports.escalationsService = exports.EscalationsService = void 0;
...
class EscalationsService {
    async findAll(company_id, filters = {}, page = 1, limit = 50) {
        ...
        const [esc, countResult] = await Promise.all([
         ...
         WHERE ${where}
         ORDER BY e.created_at DESC LIMIT ${limit} OFFSET ${offset}`, params),
            (0, database_1.query)(`SELECT COUNT(*) FROM escalations e WHERE ${where}`, params),
        ]);
    ...
    async findById(id, company_id) {
       ...
       JOIN users u1 ON u1.id = e.escalated_by
       JOIN users u2 ON u2.id = e.escalated_to
       WHERE e.id = $1 AND e.company_id = $2`, [id, company_id]);
        if (!result.rows[0])
            throw new Error('Escalation not found');
        const actions = await (0, database_1.query)('SELECT * FROM escalation_actions WHERE escalation_id = $1 ORDER BY created_at', [id]);
        return { ...result.rows[0], actions: actions.rows };
    }
    async resolve(id, company_id, user_id, resolution_notes) {
       ...
       VALUES ($1,$2,$3,'resolved',$4,$5)`, [(0, uuid_1.v4)(), id, company_id, resolution_notes, user_id]);
        await eventBus_1.eventBus.emitEvent(eventBus_1.EVENTS.ESCALATION_RESOLVED, { escalation_id: id, company_id, resolved_by: user_id });
        return { message: 'Escalation resolved successfully' };
    }
    async acknowledge(id, company_id, user_id) {
        await (0, database_1.query)(`UPDATE escalations SET status = 'acknowledged', acknowledged_at = NOW(), updated_at = NOW() WHERE id = $1 AND company_id = $2`, [id, company_id]);
        await (0, database_1.query)(`INSERT INTO escalation_actions (id, escalation_id, company_id, action_type, description, taken_by) VALUES ($1,$2,$3,'acknowledged','Escalation acknowledged',$4)`, [(0, uuid_1.v4)(), id, company_id, user_id]);
        return { message: 'Escalation acknowledged' };
    }
    async addAction(id, company_id, user_id, data) {
        const escalation = await (0, database_1.query)('SELECT * FROM escalations WHERE id = $1 AND company_id = $2', [id, company_id]);
        if (!escalation.rows[0])
            throw new Error('Escalation not found');
        if (escalation.rows[0].status === 'resolved' || escalation.rows[0].status === 'closed') {
    ...
    async getActions(id, company_id) {
        const escalation = await (0, database_1.query)('SELECT * FROM escalations WHERE id = $1 AND company_id = $2', [id, company_id]);
        if (!escalation.rows[0])
            throw new Error('Escalation not found');
        const actions = await (0, database_1.query)(`SELECT ea.*, u.first_name || ' ' || u.last_name AS taken_by_name
       FROM escalation_actions ea
       JOIN users u ON u.id = ea.taken_by
       WHERE ea.escalation_id = $1 AND ea.company_id = $2
       ORDER BY ea.created_at DESC`, [id, company_id]);
        return actions.rows;
    }
    async assignEscalation(id, company_id, user_id, assigned_to) {
        const escalation = await (0, database_1.query)('SELECT * FROM escalations WHERE id = $1 AND company_id = $2', [id, company_id]);
        if (!escalation.rows[0])
            throw new Error('Escalation not found');
        if (escalation.rows[0].status === 'resolved' || escalation.rows[0].status === 'closed') {
    ...
    async updatePriority(id, company_id, user_id, priority) {
        const escalation = await (0, database_1.query)('SELECT * FROM escalations WHERE id = $1 AND company_id = $2', [id, company_id]);
        if (!escalation.rows[0])
            throw new Error('Escalation not found');
        if (escalation.rows[0].status === 'resolved' || escalation.rows[0].status === 'closed') {
...
exports.escalationsService = new EscalationsService();
