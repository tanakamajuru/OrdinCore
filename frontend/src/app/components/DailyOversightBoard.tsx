  ...
  const TriageSignal = ({ signal }: any) => (
    <div className="flex items-center justify-between p-4 bg-background border-2 border-border border-l-4 border-l-destructive hover:border-primary transition-all group">
      <div className="flex gap-4 items-center">
        ...
        <div>
          <div className="font-bold text-foreground">{signal.house_name} – {signal.signal_type}</div>
          <p className="text-sm text-muted-foreground line-clamp-1">{signal.description}</p>
        </div>
      </div>
      <div className="flex items-center gap-4">
        <span className="text-xs font-bold text-muted-foreground uppercase">{new Date(signal.entry_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })}</span>
  ...
  const PatternCard = ({ cluster }: any) => (
    <div className="p-4 bg-background border-2 border-border hover:border-primary transition-all">
       <div className="flex justify-between items-start mb-2">
         <span className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">{cluster.risk_domain}</span>
       ...
       <div className="font-bold text-foreground mb-1">{cluster.cluster_label}</div>
       <div className="flex justify-between items-center mt-4">
         <span className="text-xs bg-primary/10 text-primary px-2 py-1 font-bold">{cluster.signal_count} Signals</span>
         <button onClick={() => navigate(`/clusters/${cluster.id}`)} className="text-xs font-bold text-primary flex items-center gap-1 hover:underline underline-offset-4">
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="p-6 md:px-12 lg:px-20 pt-28">
        ...
        <div className="flex justify-between items-end mb-8 border-b-2 border-border pb-6">
          <div>
            <h1 className="text-4xl font-black text-primary tracking-tighter uppercase italic">RM Daily Oversight</h1>
          ...
          <div className="flex gap-4">
            <div className="bg-card border-2 border-border px-6 py-2 shadow-sm text-center">
              <div className="text-xs font-bold text-muted-foreground uppercase mb-1">High Risk</div>
              <div className="text-2xl font-black text-destructive">{data?.highPriority.length || 0}</div>
            </div>
            <div className="bg-card border-2 border-border px-6 py-2 shadow-sm text-center">
              <div className="text-xs font-bold text-muted-foreground uppercase mb-1">Patterns</div>
              <div className="text-2xl font-black text-warning">{data?.pattern_signals.length || 0}</div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h2 className="text-xl font-bold text-foreground uppercase tracking-tight">High Priority Signals</h2>
            </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h2 className="text-xl font-bold text-foreground uppercase tracking-tight">Emerging Patterns</h2>
            </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h2 className="text-xl font-bold text-foreground uppercase tracking-tight">Risk Candidates</h2>
            </div>
            <div className="bg-card border-2 border-border h-full min-h-[300px]">
               {data?.risk_candidates.map(c => (
                 <div key={c.id} className="p-4 border-b-2 border-border last:border-b-0 flex justify-between items-center">
                    <div>
                      <div className="font-bold text-foreground">{c.cluster_label}</div>
                      <div className="text-xs text-muted-foreground uppercase font-black mt-1">{c.risk_domain} • {c.signal_count} Signals</div>
                    </div>
                    <button 
                      onClick={() => navigate(`/risks/promote?cluster_id=${c.id}`)}
                      className="bg-primary text-primary-foreground px-4 py-2 font-bold text-sm hover:translate-x-1 transition-transform"
                    >
                      Promote to Risk
                    </button>
                 </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h2 className="text-xl font-bold text-foreground uppercase tracking-tight">Governance Actions</h2>
            </div>
            <div className="bg-card border-2 border-border h-full min-h-[300px]">
               {data?.actions.map(a => (
