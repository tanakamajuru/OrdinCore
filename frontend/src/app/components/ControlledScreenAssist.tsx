  ...
  return <>
    ...
    <aside className={`fixed inset-y-0 right-0 z-[60] w-full max-w-md bg-card border-l border-border shadow-2xl transition-transform ${open ? 'translate-x-0' : 'translate-x-full'}`} aria-label="Controlled doctrine Screen Assist" aria-hidden={!open}>
      <div className="h-full flex flex-col p-5">
        <div className="flex items-start justify-between border-b border-border pb-4"><div><h2 className="font-semibold">Screen Assist</h2><p className="text-xs text-emerald-600 flex items-center gap-1 mt-1"><LockKeyhole className="w-3.5 h-3.5" /> Controlled doctrine · Read only</p><p className="text-xs text-muted-foreground mt-1">{role.replace(/_/g, ' ')} · {screenKey}</p></div><button type="button" onClick={() => setOpen(false)} aria-label="Close Screen Assist"><X className="w-5 h-5" /></button></div>
