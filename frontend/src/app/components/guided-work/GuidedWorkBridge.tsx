...
export function GuidedWorkBridge() {
  ...
  const next = async () => {
    setLoading(true);
    try {
      const res = await apiClient.get('/guided-work/next', { params:{ exclude: currentId || undefined } });
  ...
  return <div className="fixed bottom-4 left-4 right-4 z-[80] mx-auto max-w-3xl rounded-xl border border-border bg-card shadow-lg p-3 flex flex-wrap items-center gap-3">
    ...
    <div className="flex-1 min-w-[180px]"><div className="text-sm font-semibold text-foreground">Guided Work</div><div className="text-xs text-muted-foreground">Complete the work on this existing screen, then continue. This bar does not change or close governance records.</div></div>
