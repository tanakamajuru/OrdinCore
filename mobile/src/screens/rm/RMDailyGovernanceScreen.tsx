...
export function RMDailyGovernanceScreen() {
  ...
  return <Screen>
    ...
    {signals.length > 0 && <><Banner tone="block" icon="lock" title="Publication blocked">Every outstanding signal needs an RM decision first.</Banner><Button title={`Review ${signals.length} signal(s)`} onPress={() => nav.navigate('Tabs', { screen: 'Daily Oversight' })} /></>}
