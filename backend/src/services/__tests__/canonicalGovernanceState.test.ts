...
describe('canonical governance state resolver', () => {
  it('makes a completed effective control ready when no blocker remains', () => {
    const state = deriveCanonicalGovernanceState({ ...base, actions: [{ id: 'a1', title: 'Control', status: 'Complete', effectiveness_outcome: 'Effective' }] });
  ...
  it('treats Too Early as interim and creates a direct review blocker', () => {
    const state = deriveCanonicalGovernanceState({ ...base, actions: [{ id: 'a1', status: 'Completed', effectiveness_outcome: 'Too Early To Assess' }] });
  ...
  it('normalises Resolved as closed and identifies the exact open escalation', () => {
    const state = deriveCanonicalGovernanceState({ ...base, actions: [{ id: 'a1', status: 'Completed', effectiveness_outcome: 'Effective' }], escalations: [{ id: 'e1', lifecycle_status: 'Resolved' }, { id: 'e2', lifecycle_status: 'Under Review' }] });
