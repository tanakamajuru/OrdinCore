...
async function run() {
  try {
    console.log('Cleaning up types...');
    await pool.query('DROP TABLE IF EXISTS action_effectiveness CASCADE');
    await pool.query('DROP TYPE IF EXISTS action_effectiveness CASCADE');
    await pool.query('DROP TABLE IF EXISTS risk_candidates CASCADE');
    await pool.query('DROP TYPE IF EXISTS risk_candidates CASCADE');
